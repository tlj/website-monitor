...
func NewScheduler(interval time.Duration, intervalVariationPercentage *int, hours Hours, days Days) *Schedule {
	s := &Schedule{
		Interval:                    interval,
...
func NewSchedulerFromString(input string) (*Schedule, error) {
...
func (s *Schedule) IsWithinDays(t time.Time) bool {
	if len(s.Days) == 0 {
		return true
	}
...
func (s *Schedule) IsWithinHours(t time.Time) bool {
	if len(s.Hours) == 0 {
		return true
	}
...
func (s *Schedule) IsWithinSchedule(t time.Time) bool {
	return s.IsWithinDays(t) && s.IsWithinHours(t)
}

func (s *Schedule) NextDay(from time.Time) time.Time {
	to := from
...
func (s *Schedule) NextHour(from time.Time) time.Time {
	to := from
...
func (s *Schedule) CalculateNextFrom(from time.Time) time.Time {
...
func (s *Schedule) Equal(y *Schedule) bool {
	if s == nil && y == nil {
		return true
	}
